...
class UpdateCrearHorarioAutomaticoWithBlockControl extends Migration
{
    public function up()
    {
        DB::statement('
            ...
            DECLARE
                ...
                -- NUEVAS VARIABLES PARA CONTROL DE BLOQUES
            ...
            BEGIN
                ...
                -- OBTENER CANTIDAD TOTAL DE BLOQUES DISPONIBLES
                SELECT COUNT(*) INTO v_bloques_totales 
                FROM bloques_turnos;
                ...
                FOR v_uc_record IN (
                    ...
                    BEGIN
                        ...
                        ELSE
                            ...
                            FOR v_i IN 1..4 LOOP
                                IF v_distribucion_horas[v_i] < 2 THEN
                                    v_distribucion_horas[v_i] := 2;
                                ELSIF v_distribucion_horas[v_i] > 3 THEN
                                    v_distribucion_horas[v_i] := 3;
                                END IF;
                            END LOOP;
                            ...
                            WHILE v_resto != 0 AND v_intentos < 10 LOOP
                                FOR v_i IN 1..4 LOOP
                                    IF v_resto > 0 AND v_distribucion_horas[v_i] < 3 THEN
                                        v_distribucion_horas[v_i] := v_distribucion_horas[v_i] + 1;
                                        v_resto := v_resto - 1;
                                        EXIT WHEN v_resto = 0;
                                    ELSIF v_resto < 0 AND v_distribucion_horas[v_i] > 2 THEN
                                        v_distribucion_horas[v_i] := v_distribucion_horas[v_i] - 1;
                                        v_resto := v_resto + 1;
                                        EXIT WHEN v_resto = 0;
                                    END IF;
                        ...
                        IF v_docente_asignado IS NOT NULL THEN
                            ...
                            FOR v_i IN 1..v_clases_a_crear LOOP
                                IF v_clase_creada_count >= v_clases_a_crear THEN
                                    EXIT;
                                END IF;
                                
                                v_horas_por_clase := v_distribucion_horas[v_i];
                                v_encontrado := false;
                                v_intentos := 0;
                                
                                -- NUEVA LÓGICA: BUSCAR BLOQUES CONSECUTIVOS DISPONIBLES
                                WHILE NOT v_encontrado AND v_intentos < v_max_intentos LOOP
                                    ...
                                    FROM (
                                        SELECT 
                                            ...
                                            -- VERIFICAR BLOQUES CONSECUTIVOS DISPONIBLES
                                            public.verificar_bloques_consecutivos_disponibles(
                                                e.id, 
                                                v_docente_asignado, 
                                                d.dia, 
                                                bt.id, 
                                        ...
                                        WHERE e.sede_id = v_sede_id
                                          AND e.tipo_espacio = \'AULA\'
                                          -- ⚠️ EVITAR BLOQUES FINALES QUE CAUSEN DESBORDAMIENTO
                                          AND bt.id <= v_bloques_totales - v_horas_por_clase + 1
                                          AND public.verificar_bloques_consecutivos_disponibles(
                                                e.id, v_docente_asignado, d.dia, bt.id, 
                                                v_horas_por_clase, p_nuevo_trimestre_id, 
                                                v_lapso_academico, p_nuevo_horario_id
                                          ) = true
                                          AND NOT EXISTS (
                                            SELECT 1 
                                            FROM clases c
                                    ...
                                    IF v_dia_asignado IS NOT NULL AND v_bloque_asignado IS NOT NULL AND v_espacio_asignado IS NOT NULL THEN
                                        -- CREAR LA CLASE CON DURACIÓN EXTENDIDA
                                        INSERT INTO clases (
                                            sede_id, pnf_id, trayecto_id, trimestre_id, 
                                            unidad_curricular_id, docente_id, espacio_id, 
                                            bloque_id, horario_id, dia, duracion, created_at, updated_at
                                        ...
                                        v_advertencias := array_append(v_advertencias, 
                                            ...
                                            \' - Clase \' || v_i || \'/\' || v_clases_a_crear ||
                                            \' - Docente: \' || v_docente_asignado ||
                                            \' - Espacio: \' || v_espacio_asignado ||
                                            \' - Día: \' || v_dia_asignado || 
                                            \' - Bloque inicio: \' || v_bloque_asignado ||
                                            \' - Duración: \' || v_horas_por_clase || \' horas\'
                                        );
                                    END IF;
                                ...
                                IF NOT v_encontrado THEN
                                    v_advertencias := array_append(v_advertencias, 
                                        \'No se pudo asignar la clase \' || v_i || \' de \' || v_clases_a_crear || 
                                        \' para UC: \' || v_uc_record.uc_nombre || 
                                        \' (se necesitan \' || v_horas_por_clase || \' bloques consecutivos)\'
                                    );
                                END IF;
                            END LOOP;
    ...
    public function down()
    {
        ...
        DB::statement('
            ...
            AS $function$
            DECLARE
                -- [Versión anterior de la función - mantener la que tenías antes]
                -- ... código de la versión anterior ...
            $function$
        ');
    }
}